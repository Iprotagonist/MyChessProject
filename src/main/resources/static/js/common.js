...
com.saveUserFight = function (Level) {
    var nowTime = com.getNowTime();
    $.ajax({
        url: "/savePlayAjaxJson",//你在后台处理的地址
        type: "post",//一般用post 地址提交用 get
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        data: "{\"startTime\":\"" +nowTime+ "\",\"fightLevel\":\"" + Level+ "\"}"//json相当于一个map
    });
};
...
window.onload = function () {
    ...
    com.get("tyroPlay").addEventListener("click", function (e) {
        if (confirm("确认开始新手级对弈？")) {
            ...
            play.depth = 3;
            com.saveUserFight("新手");
            play.init();
        }
    })
    com.get("mediumPlay").addEventListener("click", function (e) {
        if (confirm("确认开始中级对弈？")) {
            play.isPlay = true;
            com.get("chessRight").style.display = "none";
            com.get("moveInfo").style.display = "block";
...
com.createMove = function (map, x, y, newX, newY) {
    ...
    // var size = x+y+newX+newY;
    // if(size == "4948"){
    //     var stopTime = com.getNowTime();
    //     $.ajax({
    //         url: "/setWin",//你在后台处理的地址
    //         type: "post",//一般用post 地址提交用 get
    //         contentType: "application/json;charset=UTF-8",
    //         dataType: "json",
    //         data: "{\"stopTime\":\"" +stopTime+ "\",\"isWin\":\"" + "胜"+ "\"}"//json相当于一个map
    //     });
    // }
        $.ajax({
            url: "/testAjaxUrlJson",//你在后台处理的地址
            type: "post",//一般用post 地址提交用 get
            contentType: "application/json;charset=UTF-8",
            dataType: "json",//一般用json格式
            // data: {"size": x + y + newX + newY, "text": h},//json相当于一个map
            data: "{\"size\":\"" + x + y + newX + newY + "\",\"text\":\"" + h + "\",\"man_my\":\"" + my + "\"}",//json相当于一个map

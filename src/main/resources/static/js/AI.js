...
AI.init = function(pace){
    ...
    // if(((pace.length + 1) % 4)%2 == 1){
    //     var newPace = [];
    //     newPace = pace.substr(pace.length  - 4,pace.length + 1);
    //     com.get("moveInfo").innerHTML='<h3>我的走法：</h3>'+
    //         com.createMove(com.arr2Clone(play.map),newPace[0],newPace[1],newPace[2],newPace[3])
    // }
    if (bill.length){
        var len=pace.length;
        var arr=[];
        ...
        if (arr.length){
            ...
            AI.historyBill = arr ;
            var qipu = arr[inx].slice(len,len+4).split("").toString();
            com.get("moveInfo").innerHTML=$("#moveInfo").html() + "<br/>"+'<h3>棋谱最佳走法：</h3>' +
                 com.createMove(com.arr2Clone(play.map),qipu[0],qipu[2],qipu[4],qipu[6]);
    ...
    if (val&&val.value!=-8888) {
        ...
        com.get("moveInfo").innerHTML=$("#moveInfo").html() + "<br/>"+'<h3>AI搜索走法：</h3>' +
             com.createMove(com.arr2Clone(play.map),man.x,man.y,val.x,val.y)
            // '<br />搜索深度：'+AI.treeDepth+
            // '<br />搜索分支：'+ AI.number+'个 <br />' +
            // '最佳着法评估：'+ val.value+'分'+
            // ' <br />搜索用时：'+ (nowTime-initTime)+'毫秒'+
            // '<br />走法：'+ man.text +
            // '原坐标' + '(' + man.x + ','+ man.y +')' +
            // '新坐标' + '('+ val.x + ','+ val.y +')'

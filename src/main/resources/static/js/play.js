...
play.clickMan = function (key,x,y){
    ...
    if (play.nowManKey&&play.nowManKey != key && man.my != com.mans[play.nowManKey ].my){
        ...
        if (play.indexOfPs(com.mans[play.nowManKey].ps,[x,y])){
            ...
            com.get("moveInfo").innerHTML='<h3>我的走法：</h3>'+
                com.createMove(com.arr2Clone(play.map),pace[0],pace[1],x,y);
...
play.AIPlay = function (){
    ...
    // com.get("moveInfo").innerHTML='<h3>AI走法：</h3>原坐标' + '(' + mapKey + pace[0] + ','+ pace[1] +')' + '-'+ '新坐标' + '('+ pace[2] + ','+ pace[3]+')'
    //  + '<br />最佳走法：'+ com.createMove(com.arr2Clone(play.map),pace[0],pace[1],pace[2],pace[3])
        // '<h3>AI搜索结果：</h3>最佳着法：'+
        // com.createMove(com.arr2Clone(play.map),man.x,man.y,val.x,val.y)+
        // '<br />搜索深度：'+AI.treeDepth+'<br />搜索分支：'+
        // '<br />走法：'+ man.text + '原坐标' + '(' + man.x + ','+ man.y +')' + '-'+ '原坐标' + '('+ val.x + ','+ val.y +')'
    com.get("clickAudio").play();
...
play.showWin = function (my){
    play.isPlay = false;
    if (my===1){
        layer.alert("恭喜你，你赢了！",{icon:6});
        play.setWin(1);
    }else{
        layer.alert("很遗憾，你输了！",{icon:5});
        play.setWin(-1);
    }
}

play.setWin = function (isMy) {
    ...
    $.ajax({
        ...
        dataType: "json",
        data: "{\"stopTime\":\"" +stopTime+ "\",\"isWin\":\"" + isWin+ "\"}"//json相当于一个map
    });
}
